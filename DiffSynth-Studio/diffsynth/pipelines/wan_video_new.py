...
from lib.utils.loss_utils import downsample_mask
from lib.utils.fft_utils import get_low_or_high_fft


class WanVideoPipeline(BasePipeline):

    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None, preencode_prompt=False):
        super().__init__(
    ...
    def from_pretrained(
        ...
        # Initialize pipeline
        pipe = WanVideoPipeline(device=device, torch_dtype=torch_dtype, preencode_prompt=preencode_prompt)
    ...
    def __call__(
        ...
        return_tensor_only: Optional[bool] = False,
        prompt_target="",
        denoising_methods = "sdedit",  # "mcs" / "basic"
        mask_preserve=None,
        self_guidance_scale=0.0
    ):
        # Scheduler
        ...
        if self.preencode_prompt:
            assert prompt_embeds is not None and negative_prompt_embeds is not None, "When `preencode_prompt` is True, `prompt_embeds` and `negative_prompt_embeds` should not be None."
            assert prompt == "" and negative_prompt == "", "When `preencode_prompt` is True, `prompt` and `negative_prompt` should be None."

            inputs_posi.update({"context": prompt_embeds})
            inputs_nega.update({"context": negative_prompt_embeds})
        # Denoise
        self.load_models_to_device(self.in_iteration_models)
        models = {name: getattr(self, name) for name in self.in_iteration_models}
        
        if mask_preserve is not None:
            mask_latent = downsample_mask(mask_preserve).to(device=self.device, dtype=inputs_shared["input_latents"].dtype)
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Inference
            if denoising_methods in ["sdedit", "t2v", "mcs", "sde", "hfs", "ncsdedit"]:
                with torch.no_grad():
            ...
            elif denoising_methods in ["sc"]:
                inputs_shared["latents"].requires_grad_(True)
                noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep)
                if cfg_scale != 1.0:
                    if cfg_merge:
                        noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
            ...
            # Scheduler
            if denoising_methods in ["sdedit", "t2v"]:
                inputs_shared["latents"] = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"])
            elif denoising_methods in ["mcs"]:
                timestep_id = torch.argmin((self.scheduler.timesteps - timestep.cpu()).abs())
                # posterior = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"], to_final=True) # 我们可以先看看posterior长什么样子
                posterior = inputs_shared["latents"] - (self.scheduler.sigmas[timestep_id]) * noise_pred
                
                decode_pst = False
                if decode_pst:
                    inputs_shared["latents"] = posterior # [1, C, T', H', W']
                    break
                
                if progress_id % 4 == 0:
                    posterior = posterior * inputs_shared["input_latents"].std(dim=list(range(1, posterior.ndim)), keepdim=True) / posterior.std(dim=list(range(1, posterior.ndim)), keepdim=True)
                    
                    # annealing blending weight
                    # w越大，越接近input video
                    if mask_preserve is not None:
                        w = mask_latent * self_guidance_scale + (1 - mask_latent) * 0.0 # [ T', H', W']
                        w = w.unsqueeze(0).unsqueeze(0) # [1, 1, T', H', W']
                        posterior = inputs_shared["input_latents"] * w + posterior * (1 - w)
                    else:
                        w = self.scheduler.sigmas[timestep_id] * 0.05 # monotonic decreasing weight
                        # w = 1 * 0.02  # constant weight
                        posterior = inputs_shared["input_latents"] * w + posterior * (1 - w)
            
                if decode_pst:
                    inputs_shared["latents"] = posterior
                    break
                
                # next step
                if progress_id + 1 < len(self.scheduler.timesteps):
                    inputs_shared["latents"] = posterior +self.scheduler.sigmas[timestep_id+1] * noise_pred
                else:
                    inputs_shared["latents"] = posterior

            if denoising_methods in ["sc"]:
                scale = 0.3
                timestep_id = torch.argmin((self.scheduler.timesteps - timestep.cpu()).abs())
                out_sample = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"])
                posterior = out_sample - (self.scheduler.sigmas[timestep_id]) * noise_pred
                difference = inputs_shared["input_latents"] - posterior
                norm = torch.linalg.norm(difference, dim=[1, 2, 3])
                norm = norm.sum()
                import ipdb; ipdb.set_trace()
                norm_grad = torch.autograd.grad(outputs=norm, inputs=inputs_shared["latents"])[0].detach()

                inputs_shared["latents"] = out_sample.detach()
                inputs_shared["latents"] -= norm_grad * scale
            
            if denoising_methods in ["sde"]:
                timestep_id = torch.argmin((self.scheduler.timesteps - timestep.cpu()).abs())
                posterior = inputs_shared["latents"] - (self.scheduler.sigmas[timestep_id]) * noise_pred
                noise_t = inputs_shared["latents"] + (1- self.scheduler.sigmas[timestep_id]) * noise_pred
                
                # update the posterior
                if self_guidance_scale > 0  and progress_id <= len(self.scheduler.sigmas)*1:
                    if mask_preserve is not None:
                        w = mask_latent*self_guidance_scale + (1 - mask_latent)* 0.0 # [ T', H', W']
                        w = w.unsqueeze(0).unsqueeze(0) # [1, 1, T', H', W']
                        posterior_update = inputs_shared["input_latents"] * w + posterior * (1 - w)
                    else:
                        posterior_update = posterior - (inputs_shared["input_latents"] - posterior) * self_guidance_scale
                    posterior = posterior_update * (1-self.scheduler.sigmas[timestep_id]) + posterior * self.scheduler.sigmas[timestep_id]
                
                # renoise
                if progress_id + 1 < len(self.scheduler.timesteps):
                    noise_t = torch.randn_like(noise_t) #* torch.sqrt(1-self.scheduler.sigmas[timestep_id+1]) + noise_t * torch.sqrt(self.scheduler.sigmas[timestep_id+1])
                    inputs_shared["latents"] = noise_t * (self.scheduler.sigmas[timestep_id+1]) + posterior * (1-self.scheduler.sigmas[timestep_id+1])
                else:
                    inputs_shared["latents"] = posterior
                    
            if denoising_methods in ["ncsdedit"]:
                timestep_id = torch.argmin((self.scheduler.timesteps - timestep.cpu()).abs())
                posterior = inputs_shared["latents"] - (self.scheduler.sigmas[timestep_id]) * noise_pred
                noise_t = inputs_shared["latents"] + (1- self.scheduler.sigmas[timestep_id]) * noise_pred
                
                # update the posterior, calibrating with the reference
                posterior_update = posterior - get_low_or_high_fft(posterior, scale=4, is_low=False) + get_low_or_high_fft(inputs_shared["input_latents"], scale=4, is_low=False)
                
                # euler step, with tweedie formula
                if progress_id + 1 < len(self.scheduler.timesteps):                  
                    inputs_shared["latents"] = noise_t * (self.scheduler.sigmas[timestep_id+1]) + posterior * (1-self.scheduler.sigmas[timestep_id+1])
                else:
                    inputs_shared["latents"] = posterior

            if denoising_methods in ["hfs"]:
                timestep_id = torch.argmin((self.scheduler.timesteps - timestep.cpu()).abs()) # self.scheduler.sigmas is decreasing
                noisy_ref = inputs_shared["input_latents"]* (1-self.scheduler.sigmas[timestep_id]) + torch.randn_like(inputs_shared["input_latents"]) * (self.scheduler.sigmas[timestep_id])
                
                # euler step
                if progress_id + 1 < len(self.scheduler.timesteps):  
                    # update the latent (extracting high-frequency from reference)
                    latent_update = inputs_shared["latents"] + get_low_or_high_fft(noisy_ref, scale=4, is_low=False) - get_low_or_high_fft(inputs_shared["latents"], scale=4, is_low=False)
                
                    inputs_shared["latents"] = latent_update + (self.scheduler.sigmas[timestep_id+1]-self.scheduler.sigmas[timestep_id]) * noise_pred
                else:
                    inputs_shared["latents"] = inputs_shared["latents"] + (- self.scheduler.sigmas[timestep_id]) * noise_pred
        ...
        with torch.no_grad():
            ...
            video_tensor = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video = None
            if not return_tensor_only:
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        if not isinstance(input_video, torch.Tensor): 
            ...
            assert len(input_video.shape) == 5
            input_video = input_video.to(dtype=pipe.torch_dtype) # [bs, 3, T, H, W]
