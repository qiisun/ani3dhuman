...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        # Scheduler
        if denoising_methods in ["mcs", "sdedit"]:
        ...
        if mask_garment is not None:
            from lib.utils.loss_utils import downsample_mask
            mask_latent = downsample_mask(mask_garment).to(device=self.device, dtype=inputs_shared["input_latents"].dtype)
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if denoising_methods in ["sdedit", "t2v"]:
                ...
                # w越大，越接近input video
                if mask_garment is not None:
                    # mask_latent = mask_garment * w
                    w = mask_latent * 0.01 + (1 - mask_latent) * 0.1 # [ T', H', W']
